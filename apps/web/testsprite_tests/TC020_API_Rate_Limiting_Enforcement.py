...
async def run_test():
    ...
    try:
        ...
        # -> Click on Login to proceed to authentication for rate limit testing.
        ...
        # -> Input email and password, then rapidly send successive login requests to test rate limiting.
        frame = context.pages[-1]
        ...
        try:
            ...
            raise AssertionError('Test case failed: The server did not respond with the expected rate limit exceeded error (HTTP 429) after rapid successive API requests as per the test plan.')
